...
def send_notification(prayer_time, notification_time, chat_id):
    # Здесь можно добавить логику отправки уведомления
    # пользователю в выбранные временные интервалы
    pass
...
def main():
    ...
    def handle_message(message):
        ...
        send_notification(prayer_time, 5, message.chat.id) # Отправка уведомления за 5 минут до намаза
        send_notification(prayer_time, 10, message.chat.id) # Отправка уведомления за 10 минут до намаза
        send_notification(prayer_time, 15, message.chat.id) # Отправка уведомления за 15 минут до намаза
